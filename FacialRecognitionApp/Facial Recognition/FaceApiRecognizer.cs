    ...
    class FaceApiRecognizer : IFaceRecognizer
    {
        ...
        public async Task<bool> CreateWhitelistFromFolderAsync(string whitelistId, StorageFolder whitelistFolder = null, IProgress<int> progress = null)
        {
            ...
            try
            {
                ...
                {
                    whitelistFolder = await KnownFolders.PicturesLibrary.GetFolderAsync(GeneralConstants.WhiteListFolderName);
                }
                ...
                try
                {
                    ...
                    // TODO: Not necesary with Fixed ID White List
                ...
                // TODO: If the White List exists this step throw an Exception
                await _faceApiClient.CreatePersonGroupAsync(WhitelistId, GeneralConstants.WhiteListFolderName);
                UpdateProgress(progress, ++progressCnt);

                // TODO: If the White List exists this step is skipped (no internal White List is created)
        ...
        public async Task<List<string>> FaceRecognizeAsync(StorageFile imageFile)
        {
            ...
            foreach(var result in identificationResults)
            {
                if(result.Candidates.Length > 0)
                {
                    ...
                    Debug.WriteLine("Local Whitelist - Name: " + personName);

                    var person = await _faceApiClient.GetPersonAsync(GeneralConstants.FixedWhiteListID, result.Candidates[0].PersonId);
                    var name = person.Name;
                    Debug.WriteLine("Candidate - Person ID : " + result.Candidates[0].PersonId);
                    Debug.WriteLine("Candidate - Confidence: " + Math.Round(result.Candidates[0].Confidence * 100, 1) + "%");
                    Debug.WriteLine("GET Person - Person.ID: " + person.PersonId);
                    Debug.WriteLine("GET Person - Pers.Name: " + person.Name);
                    recogResult.Add(name);
                }
            }
