...
namespace FacialRecognitionDoor
{
    public sealed partial class MainPage : Page
    {
        ...
        // Message's Flags
        private bool IsVisitorsMessageFirstTime = true;
        private bool IsReadyMessageFirstTime = true;
        ...
        private async Task DoorbellPressed()
        {
            ...
            if (webcam.IsInitialized() && initializedOxford)
            {
                ...
                else
                {
                    ...
                    await speech.Read(SpeechContants.VisitorNotRecognizedMessage, 3.0);
                }
            }
            else
            {
        ...
        {
            // Greet visitor
            await speech.Read(SpeechContants.GeneralGreetingMessage(visitorName), 3.0);
        ...
        private async void UpdateWhitelistedVisitors()
        {
            ...
            if (!currentlyUpdatingWhitelist)
            {
                ...
                if (IsReadyMessageFirstTime)
                {
                    await speech.Read(SpeechContants.ProjectReadyMessage, 3.0);
                    IsReadyMessageFirstTime = false;
                }
            }
        }
        ...
        private async Task UpdateWhitelistedVisitorsList()
        {
            ...
            if (IsVisitorsMessageFirstTime)
            {
                await Task.Delay(TimeSpan.FromSeconds(3.0));
                await speech.Read(SpeechContants.AddVisitorsMessage, 2.0);
            }
            ...
            foreach (StorageFolder folder in subFolders)
            {
                ...
                if (IsVisitorsMessageFirstTime)
                {
                    await speech.Read(visitorName, 1.0);
                }
            }

            IsVisitorsMessageFirstTime = false;
        }
